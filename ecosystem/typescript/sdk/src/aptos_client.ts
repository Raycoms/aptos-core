...
export class AptosClient {
  ...
  private async fetchABICascaded(address: string, result: Map<string, Gen.MoveStruct>) {
    ...
    structMap.forEach((val, key) => {
      ...
      val.fields.forEach((f) => {
        try {
          let typeTag = new TypeTagParser(f.type).parseTypeTag({ skipTypeArg: true });
  ...
  private decodeBCSVal(de: BCS.Deserializer, typeTag: TypeTag): any {
    ...
    if (typeTag instanceof TypeTagVector) {
      if (typeTag.value instanceof TypeTagU8) {
        return HexString.fromUint8Array(de.deserializeBytes()).hex();
      }

      return this.decodeBCSVal(de, typeTag.value);
    }
    if (typeTag instanceof TypeTagStruct && this.isTypeTagString(typeTag)) {
  ...
  private decodeBCSResponse(
    struct: Gen.MoveStruct,
    de: BCS.Deserializer,
    ...
    struct.fields.forEach((field) => {
      ...
      const typeTag = typeTagParser.parseTypeTag({ skipTypeArg: true });
      if (typeTag instanceof TypeTagStruct && !this.isTypeTagString(typeTag)) {
        // eslint-disable-next-line no-param-reassign
        result[field.name] = {};
        this.decodeBCSResponse(structMap.get(typeTag.fullName), de, structMap, result[field.name]);
      } else {
        // eslint-disable-next-line no-param-reassign
        result[field.name] = this.decodeBCSVal(de, typeTag);
      }
    });
  }
  ...
  async getAccountResource(
    ...
    const resourceABI = structMap.get(resource.fullName);

    const result = {};
    this.decodeBCSResponse(resourceABI, de, structMap, result);
